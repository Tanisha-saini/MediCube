import { app, database, auth,onAuthStateChanged,getDocs,query,where, collection, addDoc } from "./config.js";
...
checkusertype.addEventListener('click',(e)=>{
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            const q = query(collection(database, "labs"), where("uid", "==", user.uid));
            getDocs(q)
            .then((querySnapshot) => {
